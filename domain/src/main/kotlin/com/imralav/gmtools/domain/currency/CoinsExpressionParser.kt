    ...
    fun parse(): CoinsExpression {
        ...
        return when (operation) {
            "+" -> {
                ...
                SumExpression(leftOperand.getOrElse { Coins() }, rightOperand.getOrElse { Coins() })
            }
            "-" -> {
                val leftOperand = leftOperandOption.getOrElse { Either.Right(Coins()) }
                val rightOperand = rightOperandOption.getOrElse { Either.Right(Coins()) }
                DifferenceExpression(leftOperand.getOrElse { Coins() }, rightOperand.getOrElse { Coins() })
            }
            "*" -> {
                val leftOperand = leftOperandOption.getOrElse { Either.Right(Coins()) }
                val rightOperand = rightOperandOption.getOrElse { Either.Right(Coins()) }
                if (leftOperand.isLeft()) {
                    ...
                    ProductExpression(multiplicant, multiplier)
                } else {
            ...
            "/" -> {
                ...
                QuotientExpression(divident, divisor)
            }
            else -> CoinsValue(Coins())
        }
    }
